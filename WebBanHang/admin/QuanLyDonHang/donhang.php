    ...
    foreach ($DanhMucList as $item){
      ...
      echo'
          <tr>
               ...
               <td>'.$tongtien.'</td>
               <td>
                  <button class="btn btn-success" onclick ="XacNhan('.$item['id_HD'].')">Xác Nhận</button>
               </td>
           </tr>';
    }
...
<script type ="text/javascript">
      function XacNhan(id_HD)
      {
        var option = confirm('Thông Báo \n'+'Bạn thực sự có muốn xác nhận đơn hàng này không ? ')
        if(!option){
          return;
        }
        console.log(id_HD)
        $.post('xulydonhang.php',{
          'id_HD': id_HD,
          'action': 'XacNhan'
        },function(data){
            location.reload()
